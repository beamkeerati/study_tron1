...
| เขียนว่า | ช่องซ้ายขีดใส่อะไร | คืนอะไร | shape | โค้ด |
| --- | --- | --- | --- | --- |
| $\mu_\theta(s)$ | — | เวกเตอร์ค่ากลาง | `(B,8)` | `self.actor(obs)` |
| $\pi_\theta(\cdot \mid s)$ | **ว่าง** (จุดกลม) | **object** ไม่ใช่ตัวเลข | — | `Normal(mean, std)` |
| $a \sim \pi_\theta(\cdot \mid s)$ | ว่าง แล้วสุ่ม | action จริง | `(B,8)` | `.sample()` |
| $\pi_\theta(a \mid s)$ | **ใส่ $a$ ที่สุ่มได้แล้ว** | **scalar** | `(B,)` | `.log_prob(a).sum(-1)` |
