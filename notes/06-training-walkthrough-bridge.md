...
| $\pi_\theta$ | policy | network + วิธีสุ่ม | weight 187,272 ตัว | "ตัวตัดสินใจทั้งก้อน" | — | 2.1 |
...
| $\pi$ vs $\pi(a\mid s)$ | ก้อน network ทั้งหมด vs ตัวเลขเดียวที่คำนวณจากมัน — โน้ต 05 หัวข้อ 2.3 |
...
หลักการเดียวกับ $f$ กับ $f(x)$: $\pi_\theta(\cdot\mid s)$ คือ "ตัวฟังก์ชัน" ส่วน $\pi_\theta(a\mid s)$
คือ "ค่าของฟังก์ชันที่จุด $a$" **จุดกลมคือช่องว่างที่ยังไม่ได้เติม**
...
| --- | --- | --- |
| พูดถึง | **ค่า** ณ จุด $a$ จุดหนึ่ง | **ตัวก้อน** ทั้งอัน |
...
Tron1 และทุกอันต้องการก้อนทั้งอัน:
...
- **entropy** — "ระฆังนี้กว้างแค่ไหน" เป็นสมบัติของทั้งก้อน จุดเดียวตอบไม่ได้
- **KL** — "สองก้อนนี้ต่างกันแค่ไหน" ต้องมีสองก้อนมาเทียบ

เป็นเหตุผลเดียวกับที่เราเขียน $f' = \cos$ แทนที่จะเขียน "$f(x) = \sin(x)$ ทุก $x$ แล้ว…" ·
และมันตรงกับโค้ดพอดี: สร้างก้อนครั้งเดียวแล้วถามหลายคำถามกับก้อนเดิม

```python
dist = Normal(mean, std)        # ← รูปย่อ : สร้าง "ก้อน" ครั้งเดียว
dist.sample()                   # ↘
dist.log_prob(a).sum(-1)        #  ↘ เติมจุดกลมด้วยคำถามต่าง ๆ กัน
dist.entropy().sum(-1)          # ↗
```

> **กฎอ่านที่ใช้ได้ทันที** — เจอ $\cdot$ ในวงเล็บ ให้แปลว่า *"ก้อนทั้งอัน"* แล้วมองหาว่าประโยค
> นั้นจะเอาก้อนไปทำอะไร (สุ่ม? วัดความกว้าง? เทียบกับก้อนอื่น?) · เจอตัวแปรจริงในวงเล็บ
...
| $\cdot$ | จุดกลาง | **ช่องว่างที่ยังไม่เติม** = ก้อนทั้งอัน (§3.5.2) | ไม่ใช่เครื่องหมายคูณ |
